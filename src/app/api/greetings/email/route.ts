/**
 * API Route for sending greeting emails with batching support
 ...
 *   failedContacts: Array<{ email: string; name: string; error: string }>
 * }
 */

import { NextRequest, NextResponse } from 'next/server'
import { emailService } from '@/lib/email/service'
import { replacePlaceholders, type TemplateVariables } from '@/lib/email/templates'
...
async function processEmailBatch(
  batch: Array<{ email: string; name: string }>,
  customMessage?: string
): Promise<Array<{ email: string; name: string; result: { success: boolean; error?: string }; messageContent: string }>> {
  const results = []
  ...
  for (const contact of batch) {
    try {
      ...
      if (customMessage) {
        ...
        // For email, we need to create HTML and text versions
        const htmlMessage = messageContent.replace(/\n/g, '<br>')
        ...
        // Get default template text for logging
        const { getMonthlyGreetingTemplate } = await import('@/lib/email/templates')
        const template = getMonthlyGreetingTemplate(variables)
        messageContent = template.text || template.html.replace(/<[^>]*>/g, '')
        
        result = await emailService.sendGreetingEmail(contact.email, contact.name)
      }
  ...
  try {
    ...
    if (validContacts.length === 0) {
      return NextResponse.json(
        { 
          success: false, 
          error: 'No valid contacts provided. Each contact must have email and name.' 
        },
        { status: 400 }
      )
    }
    ...
    const allResults: Array<{ email: string; name: string; result: { success: boolean; error?: string }; messageContent: string }> = []
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const batchResults = await processEmailBatch(batch, message)
    ...
    try {
      ...
      const logs: GreetingLogInsert[] = allResults.map((r) => ({
        ...
        message_content: r.messageContent || 'Default email template',
    ...
    console.error('[Greetings API] Error sending greeting emails:', error)
    return NextResponse.json(
      {
        success: false,
